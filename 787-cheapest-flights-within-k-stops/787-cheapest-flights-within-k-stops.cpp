...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        // we can peform a Dijkstra alogrithm to see the shortest path
        // but since we have a constraint on no of stop
        // we will check after every pop operation to see if we have reached our des before k<0
        priority_queue<vector<int>,vector<vector<int>>,greater<vector<int>>> q; // min heap
        // values in form {price,node_val,k}
